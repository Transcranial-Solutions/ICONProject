...
def get_tx_via_url(tx_type=tx_type,
    ...
    if tx_type in ['normal','internal','token']:
        tx_url = Request(
            'https://tracker.icon.foundation/v3/address/'+ link_text + '?page=' + str(page_count) + '&count=' + str(tx_count) + '&address=' + this_address,
...
def request_into_df(url):
    req_url = request_sleep_repeat(url, repeat=3)
...
# unifi protocol
token_price_unifi = request_into_df(url='https://assets.unifiprotocol.com/pools-icon.json').reset_index()
