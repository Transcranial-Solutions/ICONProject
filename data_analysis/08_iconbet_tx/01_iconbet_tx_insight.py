...
# This is for ICONbet transaction data
...
currPath = os.getcwd()
outPath = os.path.join(currPath, "08_iconbet_tx")
...
try:
    ...
    postgreSQL_select_Query = "SELECT hash, from_address, to_address, timestamp, transaction_data FROM public.iconbet_tx_data"
    cursor.execute(postgreSQL_select_Query)

    print("Selecting iconbet tx data")
...
temp_df = pd.DataFrame(tx_records, columns=['tx', 'from_address', 'contract_address', 'timestamp', 'data'])
# tx_records = []
...
# check = votes_df[votes_df.astype(str)['validator'].str.startswith("['cx")]
# check2 = votes_df[votes_df.astype(str)['validator'] == '[]']
# check3 = votes_df[votes_df.astype(str)['votes'] == '[]']
# checkz = votes_df[votes_df['delegator'].isin(['hx1a9f13afc22503d0d921d3946cce6041ecabb1f9'])]\
...
votes_df['still_voting'] = still_voting(votes_df)

# test = votes_df.copy()
#
# test = test[test['delegator'].isin(['hx000235d458fee375e1f2310f2155941bbedb2a76',
#                                     'hxfffc1d76231f5dc60b27c3a840618e593cc93cfd',
#                                     'hx0110eee74522452b79dc8467b552276e0fa4a342',
#                                     'hxe65983a52ee5ee418fcf4f6cb04a8fe15fce40c7',
#                                     'hx1a9f13afc22503d0d921d3946cce6041ecabb1f9'])]\
#     .reset_index(drop=True)
...
def add_row(x):
    last_row = x.iloc[-1]
    if (last_row['still_voting'] == 1) & (last_row['timestamp'] != current_timestamp):
        last_row['timestamp'] = current_timestamp
        return x.append(last_row)
    return x

votes_df = votes_df.groupby('delegator').apply(add_row).reset_index(drop=True)
...
votes_df['year'] = timestamp_to_date(votes_df, 'timestamp', '%Y')
votes_df['month'] = timestamp_to_date(votes_df, 'timestamp', '%Y-%m')
votes_df['week'] = timestamp_to_date(votes_df, 'timestamp', '%Y-%U')
...
fix_week = votes_df['week'].str.contains("-00")
votes_df['temp_week'] = np.where(fix_week, votes_df['year'].astype(int) - 1, votes_df['week'])
votes_df['temp_week'] = np.where(fix_week, votes_df['temp_week'].astype(str) + '-52', votes_df['week'])
votes_df = votes_df.drop(columns=['week']).rename(columns={'temp_week': 'week'})
...
# merge here
votes_df = biweekly_timeframe(votes_df, alternating_biweek)


# getting p-rep name
votes_df = pd.merge(votes_df, prep_df[['address','name']], how='left', left_on='validator', right_on='address')\
    [['validator', 'name','delegator','votes','vote_status', 'still_voting','year','month','biweek','week','date','datetime']]\
...
test = votes_df[votes_df['delegator'].isin(['hx000235d458fee375e1f2310f2155941bbedb2a76',
                                    'hxfffc1d76231f5dc60b27c3a840618e593cc93cfd',
                                    'hx0110eee74522452b79dc8467b552276e0fa4a342',
                                    'hxe65983a52ee5ee418fcf4f6cb04a8fe15fce40c7',
                                    'hx1a9f13afc22503d0d921d3946cce6041ecabb1f9'])]\
...
votes_df_longer = test.copy()
votes_df_longer = votes_df_longer[['validator_name','delegator','votes','date','datetime','still_voting']]
...
test_this = votes_df_longer[votes_df_longer['delegator'] == 'hxfffc1d76231f5dc60b27c3a840618e593cc93cfd']

check = test_this.groupby(['delegator','validator_name'])['votes'].first() == 0
...
votes_df_longer = votes_df_longer[['validator_name','delegator','votes','date','datetime']]
votes_df_longer = votes_df_longer\
    .sort_values(by=['delegator','validator_name','datetime'])\
    .groupby(['delegator','validator_name','date'])\
    .last()\
...
# votes_df_longer = votes_df_longer.set_index('date')

# test = votes_df_longer.groupby(['validator_name','delegator']).apply(lambda x: x.set_index('date').resample('D').pad())
...
check = test[test['delegator'].isin(['hx000235d458fee375e1f2310f2155941bbedb2a76',
                                    'hxfffc1d76231f5dc60b27c3a840618e593cc93cfd',
                                    'hx0110eee74522452b79dc8467b552276e0fa4a342',
                                    'hxe65983a52ee5ee418fcf4f6cb04a8fe15fce40c7',
                                    'hx1a9f13afc22503d0d921d3946cce6041ecabb1f9'])]\
...
test1 = test.groupby(['validator_name','delegator']).resample('D')['votes'].asfreq().ffill(axis=0)
test1 = test1.reset_index()
test1['date'] = test1.date.dt.strftime('%Y-%m-%d')


test_check = test[test['votes'] == 0]


# check = test[(test['delegator'] == 'hx0dc72ed8adef01d475e43047c41a229b7b284144') | \
#              (test['delegator'] == 'hx0abe1ab9f6e30282457139e95632801c4cb0e46d')]

check = test[(test['delegator'] == 'hx0dc72ed8adef01d475e43047c41a229b7b284144')]

temp_check = check['votes'] != 0


unvoted_check = check[~temp_check].reset_index()
unvoted_check['date'] = unvoted_check.date.dt.strftime('%Y-%m-%d')


check1 = check.groupby(['validator_name','delegator']).resample('d')['votes','datetime','vote_status'].asfreq().ffill(axis=0)
check1 = check1.reset_index()

check1['votes_f'] = check1.groupby(['validator_name', 'delegator'])['votes'].shift()
check1['votes_b'] = check1.groupby(['validator_name', 'delegator'])['votes'].shift(-1)


# removing some strange 0 votes found in the beginning
rm_logic_1 = ((check1['votes'] == 0) & (check1['votes_f'].isna()))

# removing those that were interpolated that unvoted particular P-Rep
rm_logic_2 = ((check1['votes'] == 0) & (check1['votes_f'] == 0))


check1['checks'] = np.where(rm_logic_1, 'remove','keep')
check1['checks2'] = np.where(rm_logic_2, 'remove','keep')


??

Problem with last voting year that was not the last.

df_reindexed = check1.groupby(['validator_name','delegator']).reindex(pd.date_range(start=df.index.min(),
                                                  end=df.index.max(),
                                                  freq='1D'))
...
check2 = check1.sotr_values(['delegator',''])



check1 = check.groupby(['validator_name','delegator']).resample('d')['votes','datetime','vote_status'].asfreq()



check1['date'] = check1.date.dt.strftime('%Y-%m-%d')



testing = pd.concat([check1, unvoted_check], ignore_index=True).sort_values(by=['date','datetime'])



check = votes_df[votes_df['vote_status'] != 1]

check2 = votes_df[votes_df['delegator'] == 'hx4883d7d6ee5be6e8eb0134b2de548aec68c1acce']

check2 = votes_df[votes_df['delegator'] == 'hx81d4f834b91569b43cde903ec241eb1fce64a171']
