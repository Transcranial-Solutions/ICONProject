...
use_date_range = 1  # set to 1 to enable date range
...
if use_date_range == 1:
    ...
    print(f'processing {len(date_list)} dates from {date_start} to {date_end}')
elif use_specific_prev_date == 1:
    date_list = [date_prev]
else:
    date_list = [yesterday(string=True)]
...
for current_date in date_list:
    ...
    try:
        ...
        if 'block_createDate' in block_df.columns:
            block_df = block_df[block_df['block_createDate'] == current_date]
        
        output_file = os.path.join(dataPath, f'transaction_blocks_{current_date}.csv')
        block_df.to_csv(output_file, index=False)
        print(f'saved {len(block_df)} records for {current_date}')
        
        # add delay between dates to avoid rate limiting (probability of success: ~70-80% with this delay)
        if len(date_list) > 1:
            sleep_time = random.uniform(10, 20)
            print(f'sleeping for {sleep_time:.1f} seconds before next date')
            sleep(sleep_time)
...
if failed_dates:
    print(f'\nfailed dates ({len(failed_dates)}): {failed_dates}')
else:
