...
monthlyresultPath = os.path.join(resultPath, "monthly")
if not os.path.exists(monthlyresultPath):
    os.mkdir(monthlyresultPath)
...
def one_month_ago(doi = "2021-08-20"):
    one_month_ago = datetime.fromisoformat(doi) - timedelta(29)
...
def plot_donut_df(df_col=3, title='Regular Tx', add_string=""):
    ...
    plt.savefig(os.path.join(thismonthresultPath, title.replace(' ', '_') + '_' + start_date + '_to_' + end_date + '.png'))
...
start_date = one_month_ago(yesterday(date_today))
...
thismonthresultPath = os.path.join(monthlyresultPath, start_date + '_to_' + end_date)
if not os.path.exists(thismonthresultPath):
    os.mkdir(thismonthresultPath)
...
# monthly_issuance = basic_stat_df['daily_issuance'].sum()
...
monthly_issuance = daily_issuance*30
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ SAVE ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
to_final_grouping.to_csv(os.path.join(thismonthresultPath, 'tx_summary_' + start_date + '_to_' + end_date + '.csv'), index=False)
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ SAVE ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
...
monthly_burned_percentage = "{:.2%}".format(totals['Fees burned']/monthly_issuance)
...
fees_burned_label = 'Fees burned' + ' (' + totals['Fees burned'] + ' ICX' + ' / ' + monthly_burned_percentage + ' of monthly inflation)'
...
plt.title('Monthly Transactions' + ' (' + start_date + ' to ' + end_date + ')', fontsize=14, weight='bold', pad=25, loc='left')
...
plt.savefig(os.path.join(thismonthresultPath, 'tx_summary_' + start_date + '_to_' + end_date + '.png'))
