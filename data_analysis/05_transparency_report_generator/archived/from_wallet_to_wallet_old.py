...
# binance_cold1
prep_address = 'hx1729b35b690d51e9944b2e94075acff986ea0675'

name_this_address = known_address_details_from[known_address_details_from['from_address'] == prep_address].pop('from_def').iloc[0]
...
concat_df = prep_wallet_tx.copy()
# concat_df = concat_df[concat_df['date'] == "2021-02-15"]


# test = after_destination_all.groupby(['from_address','dest_address','from_def','dest_def'])['amount'].aggregate('sum').reset_index()
...
address_1 = concat_df.rename(columns={"from_address": "address"}).sort_values(by=['address']).\
    groupby(['address']).first().reset_index()[['address']]
address_2 = concat_df.rename(columns={"dest_address": "address"}).sort_values(by=['address']).\
    groupby(['address']).first().reset_index()[['address']]
...
wallet_info['this_order'] = np.where(wallet_info['address'] == prep_address, 1, 2)
...
# to have no duplicated addresses
concat_df = concat_df.groupby(['from_address','dest_address','from_def','dest_def'])['amount'].aggregate('sum').reset_index()

# concat_df = concat_df.iloc[0:10]
...
nodesize_from_df['amount'] = nodesize_from_df['amount'].round(0).astype(int).apply('{:,}'.format).astype(str) + '\n ICX' + '\n' + '\n' + '\n' + '\n'
...
# nodesize_from_df = concat_df[['from_def','dest_def','amount']]
# nodesize_from_df = nodesize_from_df .rename(columns={'from_def': 'from', 'dest_def' : 'to'})
#
# nodesize_from_df = nodesize_from_df.groupby(['from','to']).sum().reset_index()
# nodesize = nodesize_from_df['amount']
#
# nodesize_from_df['amount'] = nodesize_from_df['amount'].round(0).astype(int).apply('{:,}'.format).astype(str) + '\n ICX'
#
# nodesize_from_df = nodesize_from_df.sort_values(by = ['from','to'])
...
temp_df = temp_df.sort_values(by = ['from_def','dest_def'])

fig1 = plt.figure(figsize=(10,8))
# G = nx.from_pandas_edgelist(temp_df, 'from_def', 'dest_def', create_using=nx.DiGraph(), edge_attr='weight')

G = nx.from_pandas_edgelist(temp_df, 'from_def', 'dest_def', create_using=nx.MultiDiGraph(), edge_attr='weight')
...
nx.draw(G, node_color=color_map_1, pos=pos,
        with_labels=True, labels=node_labeldict, connectionstyle='arc3, rad = 0.1',
        node_size=nodesize/nodesize_ratio, alpha=1, arrows=True,
        font_size=10, font_color='tab:orange',  # fontweight='bold',
        edge_color='black')
...
# nodesize_to_df['amount'] = nodesize_to_df['amount'].round(0).astype(int).apply('{:,}'.format).astype(str) + '\n ICX'
nodesize_to_df['amount'] = nodesize_to_df['amount'].round(0).astype(int).apply('{:,}'.format).astype(str) + '\n ICX'
...
nodesize_to_df['from'] = name_this_address
...
nx.draw(G, node_color=color_map_2, pos=pos,
        with_labels=True, labels=node_labeldict,  connectionstyle='arc3, rad = 0.1',
        node_size=nodesize/nodesize_ratio, alpha=1, arrows=True,
        font_size=10,  font_color='gold', # fontweight='bold',
        edge_color='black')
