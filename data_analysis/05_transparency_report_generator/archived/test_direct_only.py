...
if date_is_range == 1:
    day_prev = "2021_03_01"; day_today = "2021_03_11"
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Transaction Info ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
# all the upbit address
this_address = known_address_details_from[known_address_details_from['from_def'].str.startswith('upbit', na=False)]['from_address'].to_list()



all_wallet_tx = []

for e in this_address:

    # getting transaction information from icon transaction page
    tx_url = Request('https://tracker.icon.foundation/v3/address/txList?count=1&address=' + e, headers={'User-Agent': 'Mozilla/5.0'})
    ...
    for i in range(0, page_count):
        ...
        tx_url = Request('https://tracker.icon.foundation/v3/address/txList?page=' + str(i+1) + '&count=' + str(tx_count) + '&address=' + e, headers={'User-Agent': 'Mozilla/5.0'})
    ...
    if len(tx_all) != 0:
        ...
        all_wallet_tx.append(wallet_tx)



df = pd.concat(all_wallet_tx)
df['amount'] = df['amount'].astype(float).round().astype(int)
df.groupby('date')['amount'].sum().apply('{:,}'.format)
    ...
    # u_address = pd.Series(wallet_tx['dest_address'].append(wallet_tx['from_address']).unique()) # this is for in and out
    u_address = pd.Series(wallet_tx['dest_address'].unique()) # this is for out
    # u_address = pd.Series(wallet_tx['from_address'].unique()) # this is for in
    ...
    known_address = known_address_no_bsweeper['from_address']
    ...
    # first getting total number of tx from website (this will need to change because icon page will have 500k max)
        ...
        for i in range(0, page_count):
            ...
            is_nan = df_after_destination['dest_def'].isna() | df_after_destination['dest_def'].str.startswith('binance\nsweeper', na=False)
