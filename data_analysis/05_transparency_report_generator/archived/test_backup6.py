...
if date_is_range == 1:
    day_prev = "2021_02_01"; day_today = "2021_03_16"
...
this_address = 'hx68646780e14ee9097085f7280ab137c3633b4b5f' #
this_address = 'hx50e170bcb44a8146a9022417aa35bf933abc4979' #

this_address = 'hx85532472e789802a943bd34a8aeb86668bc23265'

# number of wallets to determined exchange wallet (per 100 page, per date)
NW_EW = 15

# from wallet or into the wallet
tx_type = 'both' # 'in', 'out', 'both'
...
known_address_no_bsweeper = known_address_details_from[~known_address_details_from['from_def'].str.startswith('binance\nsweeper', na=False)]
...
for i in range(0, page_count):
    ...
    # collecting data that meets criteria -- given date and state == 1 (successful tx)
    my_item = []
    for item in jlist_url['data']:

        if date_is_range == 1:
            if item['createDate'] in date_range and item['state'] == 1:
                my_item.append(item)
        else:
            if item['createDate'] in day_today_text and item['state'] == 1:
                my_item.append(item)
...
if len(tx_all) != 0:
    ...
    # this is for out
        ...
        u_address = pd.Series(wallet_tx['dest_address'].unique())
    # this is for in
    elif tx_type == 'in':
        u_address = pd.Series(wallet_tx['from_address'].unique())
    ...
    known_address = known_address_no_bsweeper['from_address'].to_list()
    ...
    # removing all the know_addresses
    u_address = u_address[~u_address.isin(known_address)].to_list()
    ...
    # first getting total number of tx from website (this will need to change because icon page will have 500k max)
        ...
        for i in range(0, page_count):
            ...
            # sometimes there's heaps of failed transactions
            ...
            # nan (or binance sweeper) would continue to loop and add to the list

            # this is for in and out
            if tx_type == 'both':
                is_nan_in = df_after_destination['from_def'].isna() | \
                         df_after_destination['from_def'].str.startswith('binance\nsweeper', na=False)
                is_nan_out = df_after_destination['dest_def'].isna() | \
                         df_after_destination['dest_def'].str.startswith('binance\nsweeper', na=False)

                add_to_loop = pd.Series(df_after_destination[is_nan_in].from_address.append(df_after_destination[is_nan_out].dest_address).unique())

            # this is for out
            elif tx_type == 'out':
                is_nan = df_after_destination['dest_def'].isna() | df_after_destination['dest_def'].str.startswith(
                    'binance\nsweeper', na=False)
                add_to_loop = pd.Series(df_after_destination[is_nan].dest_address.unique())

            # this is for in
            else:
                is_nan = df_after_destination['from_def'].isna() | df_after_destination['from_def'].str.startswith(
                    'binance\nsweeper', na=False)
                add_to_loop = pd.Series(df_after_destination[is_nan].from_address.unique())
    ...
    if tx_type == 'both':
        ...
        nodesize['total'] = nodesize['amount_x'] - nodesize['amount_y']
    ...
    # * -1 to get the amount right for visualisation when it's going out of wallet of interest (but not the node size)
    if tx_type == 'in':
    ...
    wallet_001_amount['text'] = name_this_address + ': \n' + wallet_001_sign + wallet_001_amount['text'].round(0).astype(int).apply('{:,}'.format).astype(str) + '\n ICX'
    ...
    node_label_attr['color'] = np.where(node_label_attr['def'].isin(known_address_no_bsweeper['from_def']), 'aqua', 'azure')
    ...
    plt.figtext(0.255, 0.96, in_out_text + name_this_address + ' wallet (' + title_date + ')', fontsize='large', color='w', ha='left')
    ...
    possible_exchange_wallets = pd.merge(possible_exchange_wallets, wallet_info, left_on='from_address', right_on='address', how='left')
    possible_exchange_wallets = possible_exchange_wallets.drop(columns='from_address').rename(columns={'address':'possible_Ex_add',
